        ...
        with tracer.start_as_current_span("agent_team_request") as current_request_span:
            ...
            while self._tasks:
                ...
                with tracer.start_as_current_span("agent_team_task") as current_task_span:
                    ...
                    print(f"Found agent '{agent.name}' for task recipient '{task.recipient}'")
                    if agent and agent.agent_instance:
                        print(f"Processing task for agent '{agent.name}'")
                        run = self._agents_client.runs.create(
                            thread_id=self._agent_thread.id, agent_id=agent.agent_instance.id
                        )

                        print(f"run status for agent '{agent.name}': {run.status}")
                        while run.status in ["queued", "in_progress", "requires_action"]:
                            time.sleep(1)
                            run = self._agents_client.runs.get(
                                thread_id=self._agent_thread.id, run_id=run.id
                            )
                            ...
                            if run.status == "requires_action" and isinstance(run.required_action, SubmitToolApprovalAction):
                                #print(f"Run {run.id} requires submitToolApprovalAction.")
                                ...
                                if tool_calls: 
                                    ...
                                    if tool_approvals:
                                        self._agents_client.runs.submit_tool_outputs(
                                            thread_id=self._agent_thread.id, run_id=run.id, tool_approvals=tool_approvals
                                    )
                            elif run.status == "requires_action":
                                tool_outputs = []
                                for tool_call in run.required_action.submit_tool_outputs.tool_calls:
                                    if tool_call.function.name == "create_task":
                                        #print(f"Creating task for tool call: {tool_call}")
                                        ...
                                        _create_task(
                                            ...
                                            requestor=arguments["requestor"],
                                        )
                                    tool_outputs.append(ToolOutput(
                                        tool_call_id=tool_call.id,
                                        output=f"function {tool_call.function.name} call initiated",
                                    ))
                                

                                run = self._agents_client.runs.submit_tool_outputs(
                                    thread_id=self._agent_thread.id,
                                    run_id=run.id,
                                    tool_outputs=tool_outputs
                                )
                                print(f"Submitted tool outputs for run {run.id} in thread {self._agent_thread.id}")
                        ...
                        print(f"Run outputs '{agent.name}': {run.status}")
